...
void ast_node_destroy(ast_node_t* node)
{
	...
	switch (node->type)
	{
		...
		case AST_CALL:
		{
			...
			arr_ast_node_destroy(&node->as.call.arguments);
			break;
		}
		...
		case AST_ARRAY:
		{
			arr_ast_node_destroy(&node->as.array.elements);
			break;
		}

		case AST_TABLE:
		{
			arr_ast_node_destroy(&node->as.table.entries);
			break;
		}
		...
		case AST_FUNC_DEFINE:
		{
			...
			arr_ast_node_destroy(&node->as.function.parameters);
		...
		case AST_IF:
		{
			arr_ast_node_destroy(&node->as.conditional.conditions);
			arr_ast_node_destroy(&node->as.conditional.bodies);
