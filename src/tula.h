...
#define exitErr_badUsageF(format, ...) \
	(tula_exitError(TULA_EXIT_BAD_USAGE, "Bad Usage: " format, __VA_ARGS__))
...
NO_RETURN void tula_exitError(int32_t exitCode, const char* format, ...);


static NO_RETURN void exitErr_internal(const char* msg)
{
	tula_exitError(
		TULA_EXIT_UNDEF_INTERNAL_ERR,
...
static NO_RETURN void exitErr_badUsage(const char* msg)
{
	tula_exitError(
		TULA_EXIT_NO_MEM,
		"Bad Usage: %s",
...
static NO_RETURN void exitErr_noMem(void)
{
	tula_exitError(
		TULA_EXIT_NO_MEM,
...
static NO_RETURN void exitErr_earlyStateAccess(void)
{
	tula_exitError(
